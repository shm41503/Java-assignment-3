			...
			try (Scanner order = new Scanner(System.in)) {
				...
				while(true)
				{
				  ...
				  inputs.add(b.calculateCost(burger)); // adds burger price within input. tracks prices of order 
					...
					c.setType(cheese);
				    inputs.add(c.calculateCost(cheese));
					...
					while (ingredients != 0) 
					{
						...
						for(int a= 0; a < ing1.length; a++)
						{
							if(ingredients == 1) { //lettuce
					    		ingredientPrice = 0;
					    		ing1[a]=ingredientPrice;	// add new ingredients to array ing1
					    		continue;
							}
					    	if(ingredients == 2) { //tomatoes
					    		ingredientPrice = 0;
					    		ing1[a]=ingredientPrice;;
					    		continue;
					    	}
					    	if(ingredients == 3) {
					    		ingredientPrice = 50;
				    	...
				    	b.costIngredients(ing1); //calculates ingredients price in burger class 
